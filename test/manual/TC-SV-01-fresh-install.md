...
4. Open `/rgpvpw opt` and walk every panel: Options, Zones, Voice Packs, Detection Bar,
   Flash, Profiles, all twelve category panels and Enemy Avoid
...
- `PVPWarnConfiguration` contains `enableCombatStateTracking = true`,
  `lockCombatStateFrame = true`, `enableStanceStateTracking = true`,
  `lockStanceStateFrame = true`, `hideUnknownStance = false`,
  `activeVoicePack = "default"`, `lastNotifiedVersion = ""` and an empty `frames` table
