...
- Two clients in the same guild or party, one running a **newer** PVPWarn version than the
  other
- On the older client: a clean session (fresh login) and an empty/older `lastNotifiedVersion`
  in its SavedVariables
...
- The older client prints the update notice shortly after login or the roster change
