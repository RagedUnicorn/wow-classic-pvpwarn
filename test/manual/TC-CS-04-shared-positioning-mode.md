...
8. Click **Reset position** on each panel
...
- **Reset position** returns that icon to its default spot on the right edge of the target frame
  (combat flush, stance 25px further out, edge to edge), including after a drag that re-anchored
  it to `UIParent`
